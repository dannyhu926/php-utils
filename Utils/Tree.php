...
class Tree
{
    ...
    public function __construct($arr = array(), $params_alias = array())
    {
        $this->arr = $arr;
        $this->ret = '';
        if (isset($params_alias['id'])) {
            $this->param_id = $params_alias['id'];
        }
        if (isset($params_alias['parent_id'])) {
            $this->param_parent_id = $params_alias['parent_id'];
        }
        if (isset($params_alias['name'])) {
            $this->param_name = $params_alias['name'];
        }
        if (isset($params_alias['children'])) {
            $this->param_children = $params_alias['children'];
        }
    ...
    public function getParent($myid)
    {
        ...
        if (is_array($this->arr)) {
            foreach ($this->arr as $id => $a) {
                if (isset($a[$this->param_parent_id]) && ($a[$this->param_parent_id] == $pid)) {
                    $newarr[$id] = $a;
                }
            }
        }
    ...
    public function getChild($myid)
    {
        ...
        if (is_array($this->arr)) {
            foreach ($this->arr as $id => $a) {
                if (isset($a[$this->param_parent_id]) && ($a[$this->param_parent_id] == $myid)) {
                    $newarr[$id] = $a;
                }
            }
        }
